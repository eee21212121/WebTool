...
│   └── json-formatter.html
├── tutorials/
│   ├── image-compression-workflow.html
│   └── static-site-seo-checklist.html
├── data/
│   └── tools.json
├── assets/
│   ├── css/styles.css
...
实用教程页位于 `tutorials/` 目录，教程索引是 `tutorials.html`。教程适合写具体流程和检查清单，新增后同样需要更新 `sitemap.xml`。
